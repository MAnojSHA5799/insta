...
AGE_GROUPS = {
    ...
    "hair_fall": ["35-44", "45-54", "25-34"],
    "hair_gray": ["45-54", "55+", "35-44"],
    "face_wash": ["18-24", "25-34", "35-44"],
    "skincare": ["25-34", "35-44", "18-24", "45-54"],
    "general": ["18-24", "25-34", "35-44", "45-54", "55+", "Gen Z", "Millennials", "Gen X"]
}
...
CATEGORY_CONFIG = {
    "hair_growth": {
        "type": "spiky", "brands": ["Mamaearth", "Minimalist", "WOW", "Biotique", "Indulekha"],
        "keywords": ["hair serum for hair growth", "hair growth serum", "hair growth oil", "best hair growth serum"],
        "peak_times": ["6-9PM", "9-12PM"]
    },
    "hair_fall": {
        "type": "always_on", "brands": ["Mamaearth", "The Ordinary", "WOW", "Biotique", "Khadi"],
        "keywords": ["hair oil for hair fall", "hair serum for hair fall", "anti hair fall oil", "hair fall control serum"],
        "peak_times": ["6-9PM", "9-12PM", "9-12AM"]
    },
    "hair_gray": {
        "type": "spiky", "brands": ["L'Oreal", "Bigen", "Godrej", "Indus Valley", "Just Herbs"],
        "keywords": ["hair serum for hair gray", "anti gray hair serum", "gray hair treatment", "hair color serum"],
        "peak_times": ["6-9PM", "3-6PM"]
    },
    "face_wash": {
        "type": "always_on", "brands": ["Himalaya", "Cetaphil", "Ponds", "Neutrogena", "Lakme"],
        "keywords": ["best face wash", "face wash for oily skin", "face wash for dry skin", "acne face wash"],
        "peak_times": ["6-9AM", "6-9PM"]
    },
    "skincare": {
        "type": "always_on", "brands": ["Minimalist", "The Ordinary", "CeraVe", "Plum", "Dot & Key"],
        "keywords": ["best skincare products", "skincare routine", "skincare for oily skin", "anti aging cream"],
        "peak_times": ["9-12PM", "6-9PM"]
    },
    "body_lotion": {
        "type": "always_on", "brands": ["Nivea", "Vaseline", "Cetaphil", "Lakme", "Ponds"],
        "keywords": ["body lotion for dry skin", "best body lotion", "moisturizing body lotion"],
        "peak_times": ["6-9PM", "9-12PM"]
    }
}
...
st.set_page_config(page_title="🔍 Multi-Product Search Demand Analyzer", layout="wide", page_icon="📈")

def detect_multiple_categories(query):
    query_lower = query.lower()
    categories = []
    
    if any(word in query_lower for word in ["serum", "hair", "growth", "fall", "gray"]):
        if "growth" in query_lower: categories.append("hair_growth")
        elif "fall" in query_lower: categories.append("hair_fall")
        elif "gray" in query_lower: categories.append("hair_gray")
        else: categories.append("hair_growth")
    
    if any(word in query_lower for word in ["face wash", "facewash"]):
        categories.append("face_wash")
    
    if any(word in query_lower for word in ["skin care", "skincare"]):
...
def generate_platform_analysis(category):
    platform_data = []
    quick_commerce_boost = 1.4 if category in ["hair_fall", "face_wash", "skincare"] else 1.0
    for platform in PLATFORMS:
        traffic = random.randint(45000, 320000)
        conversion = random.uniform(2.8, 9.2)
        market_share = random.randint(7, 38)
...
def enhanced_parse_demand_signal(data, original_query, category):
    ...
    result.update({
        ...
        "city_demand_ranking": city_data, "top_5_demand_cities": top_cities_demand,
        "top_3_peak_times": top_times, "detected_category": category,
        "age_brackets": get_age_groups(category),
        "brand_keywords": brand_keywords, "platform_analysis": platform_data,
        "price_range_traffic": price_data, "current_season": CURRENT_SEASON
    })
    return result
...
# 🔥 MAIN STREAMLIT APP
st.title("🔍 Multi-Product Search Demand Analyzer v3.0")
st.markdown("***AI-Powered Google Search Demand Analysis | Multiple Products + City-wise Insights***")
...
query = st.sidebar.text_input("🔍 Search Query:", value="top 5 hair serum and face wash and skin care product top 15 city")
num_cities = st.sidebar.slider("🏙️ Number of Cities", 5, 25, 15)

if st.sidebar.button("🚀 ANALYZE MULTIPLE PRODUCTS", type="primary"):
    if not api_key_input:
        st.sidebar.error("👈 Enter API key first!")
    else:
        categories = detect_multiple_categories(query)
        
        if not categories:
            st.error("❌ No products detected in query. Try: 'hair serum', 'face wash', 'skincare'")
        else:
            all_results = {}
            with st.spinner(f"🔄 Analyzing {len(categories)} products..."):
                for cat in categories:
                    st.info(f"📊 Analyzing **{cat.replace('_', ' ').title()}**...")
                    data = fetch_search_demand(f"{query} {cat}")
                    if data:
                        parsed = enhanced_parse_demand_signal(data, query, cat)
                        all_results[cat] = parsed
            
            if all_results:
                # 🔥 TABS FOR EACH PRODUCT
                tabs = st.tabs([cat.replace("_", " ").upper() for cat in all_results.keys()])
                
                for i, (cat, parsed) in enumerate(all_results.items()):
                    with tabs[i]:
                        st.header(f"📊 {cat.replace('_', ' ').title()} ANALYSIS")
                        
                        # 🔥 Category & Insights
                        col1, col2, col3 = st.columns(3)
                        col1.metric("🏷️ Category", cat.upper())
                        col2.metric("👥 Target Age", ", ".join(parsed['age_brackets'][:3]))
                        col3.metric("❄️ Season", CURRENT_SEASON.upper())
                ...
                summary_data = []
                for cat, parsed in all_results.items():
                    top_city = parsed['city_demand_ranking'][0]['city']
                    top_brand = parsed['brand_keywords'][0]['keyword'][:25]
                    summary_data.append({
                        "Product": cat.replace("_", " ").title(),
                        "Top City": top_city,
                        "Top Brand": top_brand,
                        "Demand Score": parsed['city_demand_ranking'][0]['demand_score']
                    })
                
                summary_df = pd.DataFrame(summary_data)
                st.dataframe(summary_df, use_container_width=True)
                
                # 🔥 Excel Download (All Products)
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                filename = f"MULTI_PRODUCT_ANALYSIS_{query.replace(' ', '_')}_{timestamp}.xlsx"
                
                with pd.ExcelWriter("temp_multi_analysis.xlsx", engine='openpyxl') as writer:
                    sheet_num = 1
                    for cat, parsed in all_results.items():
                        pd.DataFrame(parsed['city_demand_ranking']).to_excel(writer, f'{cat.upper()}', index=False)
                        pd.DataFrame(parsed['brand_keywords']).to_excel(writer, f'{cat.upper()}_KEYWORDS', index=False)
                        sheet_num += 1
                
                with open("temp_multi_analysis.xlsx", "rb") as f:
                    st.download_button(
                        label=f"📥 Download All Excel ({filename})",
                        data=f,
                        file_name=filename,
                        mime="application/vnd.openpyxlformats-officedocument.spreadsheetml.sheet"
                    )
            else:
                st.error("❌ No data returned from API for any product")

# 🔥 Instructions
with st.expander("📖 How to Use Multi-Product Analyzer"):
    st.markdown("""
    **✅ Works with queries like:**
    - `top 5 hair serum and face wash top 15 city`
    - `skincare and hair growth serum top 10 city`
    - `facewash face wash skincare products`
    
    **🔥 Features:**
    - **Multiple tabs** for each detected product
    - **Combined summary** table at bottom
    - **Single Excel** with all product sheets
    - **Auto-detects** hair serum, face wash, skincare
    """)

st.sidebar.markdown("---")
st.sidebar.markdown("**✅ Multi-Product Analyzer v3.0**\n*Hair Serum + Face Wash + Skincare*")
